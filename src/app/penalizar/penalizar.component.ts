...
export class PenalizarComponent implements OnInit {
  ...
  obtenerReservas() {
    this.http
      .get<{ success: boolean; reservas: any[] }>(
        'http://127.0.0.1:8000/gym/GetReservas/'
      )
      .subscribe({
        next: (response) => {
          if (response.success) {
  ...
  penalizarUsuario(index: number) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.post('http://127.0.0.1:8000/gym/Penalizar/', body).subscribe({
          next: (response: any) => {
  ...
  confirmarAsistencia(index: number){
    ...
    Swal.fire({
      ...
      if(result.isConfirmed) {
        this.http.post('http://127.0.0.1:8000/gym/CrearAsistencia/', body).subscribe({ //cambie su ip
          next: (response: any) => {
            if(response.success) {
              Swal.fire({
